...
for i = 1:size(x1,2)
    ...
    plot(x1(1,i), x1(2,i), 'ko', 'MarkerSize', 3, 'LineWidth', 0.8), hold on
end
if exist('x1Inliers', 'var') && exist('x2Inliers', 'var')
    x2Inliers(1,:) = x2Inliers(1,:) + size(im1,2);
    plot([x1Inliers(1,:); x2Inliers(1,:)], [x1Inliers(2,:); x2Inliers(2,:)], 'b', 'LineWidth', 0.8,'Color',rand(1,3)), hold on
    plot(x1Inliers(1,:), x1Inliers(2,:), 'bo', 'MarkerSize', 3, 'LineWidth'), hold on
end
hold off
end
